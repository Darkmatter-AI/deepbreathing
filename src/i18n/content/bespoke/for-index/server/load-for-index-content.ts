...
import type { ForIndexContent } from "../types";

import publication from "../publication.json";

export type ForIndexContentLocale = "de-de" | "es-es" | "fr-fr" | "ja-jp" | "pt-br";
...
type Coverage = {
  ...
  locales: Record<ForIndexContentLocale, {
    publishable: boolean;
...
export async function loadForIndexContent(
  locale: ForIndexContentLocale,
): Promise<ForIndexContent> {
  ...
  if (
    ...
    throw new Error(
      `Native i18n for-index content is incomplete for ${locale}; refusing English fallback`,
    );
  }

  return contentLoaders[locale]() as Promise<ForIndexContent>;
}
