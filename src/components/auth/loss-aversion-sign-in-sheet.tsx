...
/**
 * LossAversionSignInSheet — "Conversion Prompt C (Loss Aversion)"
 *
 * Shows the user's real just-completed session (mode, duration, "just now") and
 * frames signup as not losing it. Honest by construction: no simulated social
...
interface LossAversionSignInSheetProps {
  open: boolean;
...
export function LossAversionSignInSheet({
  open,
  onOpenChange,
  onSuccess,
  sessionMode,
  ...
  const handleGoogle = async () => {
    trackEvent("signin_google_clicked", { variant: "loss_aversion" });
    try {
  ...
  const handleMagicLink = async (e: React.FormEvent) => {
    ...
    setStatus("sending");
    trackEvent("signin_magic_link_sent", { variant: "loss_aversion" });
    try {
...
.${PREFIX}-sheet{
  ...
  --coral:#f6743b;--line:rgba(255,255,255,0.10);
...
.${PREFIX}-card{display:flex;align-items:center;gap:14px;background:rgba(29,19,13,0.55);border:1px solid var(--line);border-radius:18px;padding:14px 16px;margin:0 0 20px;}
.${PREFIX}-ring-wrap{position:relative;width:44px;height:44px;flex:none;display:flex;align-items:center;justify-content:center;}
.${PREFIX}-svg{position:absolute;top:0;left:0;width:44px;height:44px;}
.${PREFIX}-glyph{position:relative;z-index:1;}
.${PREFIX}-card-text{flex:1;min-width:0;}
.${PREFIX}-eyebrow{font-size:10px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:var(--cream-40);margin:0 0 3px;}
.${PREFIX}-mode{font-size:15px;font-weight:600;color:var(--cream);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;line-height:1.2;}
.${PREFIX}-meta{display:flex;align-items:center;gap:5px;font-size:12px;color:var(--cream-58);margin-top:4px;}
...
.${PREFIX}-google{width:100%;min-height:54px;border-radius:999px;border:none;cursor:pointer;background:#fff;color:#241a13;font-family:inherit;font-size:14.5px;font-weight:600;display:flex;align-items:center;justify-content:center;gap:11px;padding:0 20px;transition:transform .18s var(--ease),box-shadow .2s,filter .2s;}
...
.${PREFIX}-g-label{font-size:14.5px;font-weight:600;line-height:1.2;}
.${PREFIX}-g-sub{font-size:11.5px;font-weight:400;color:#5c4033;line-height:1.2;}
...
.${PREFIX}-check-ring{width:58px;height:58px;border-radius:50%;background:#46d39b;display:flex;align-items:center;justify-content:center;color:#241006;box-shadow:0 12px 30px -10px rgba(70,211,155,0.45);margin-bottom:18px;}
