...
- Verify with `pnpm test`; `pnpm build` runs the native-i18n `check:*` gates and the post-build SSR tests. Browser proof: Playwright Chromium is installed (`pnpm dlx playwright@1.62.1 ...`); the verify-deepbreathing skill lives in unmerged PR #73. WebAudio stays suspended in headless Chromium, so the orb's Start->Pause transition needs a headed run or an unlocked audio context.
