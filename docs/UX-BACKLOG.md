...
- **DMARC** — ✅ **shipped 2026-07-25 at `p=none`.** `_dmarc.deepbreathingexercises.com` TXT `v=DMARC1; p=none; rua=mailto:hi@abiassi.com; fo=1`. Because the reporting address is on a different domain, RFC 7489 §7.1 also required an authorization record on the receiving side: `deepbreathingexercises.com._report._dmarc.abiassi.com` TXT `v=DMARC1` — without it Google/Microsoft/Yahoo silently decline to send reports. Both are on Vercel DNS under the `amorimferreiras-projects` scope, not `darkmatterai`. **Alignment already passes on both legs**, so tightening is safe when the data supports it: DKIM `d=` is the apex and matches `From` exactly, and SPF's envelope domain `send.deepbreathingexercises.com` shares an organizational domain with the apex under the default relaxed alignment. **Next step:** after ~2 weeks of aggregate reports arriving at `hi@abiassi.com`, confirm no legitimate sender is failing, then move to `p=quarantine`. Do not skip straight to `p=reject`.
