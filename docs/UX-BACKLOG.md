...
### 25. [P1] ✅ FIXED 2026-06-06 — Mobile: breathing orb not tappable on `/breathing-visualizer` & `/4-7-8-breathing-timer`
At 375px the orb (`button[aria-label="Start Session"]`, z-20) was covered by the hero-content overlay (`div … inset-y-0 max-w-xl justify-end`, z-30) which **intercepted pointer events** — tapping the orb (the only in-page start control) did nothing. Reproducible 2/2 on both pages, confirmed by the browser's hit-testing. Desktop was unaffected; home/`/breathe/box`/`/breathe/tummo` orbs always worked (the `/breathe` template already used the correct overlay), so it was specific to these two bespoke tool-page heroes; it cascaded to `/ja/4-7-8-breathing-timer`. **Fix:** adopted the proven `pattern-page.tsx` overlay on both tool pages — `pointer-events-none` on the overlay + `pointer-events-auto` on the inner content, and mobile `inset-x-0 bottom-0` (bottom strip) instead of full-height `inset-y-0` so it no longer covers the orb (desktop keeps `sm:inset-y-0 sm:justify-center`). **Verified locally** (dev server, no proxy needed): orb starts the animation at 375px on both pages, desktop unchanged, in-overlay CTAs (Start session → /breathe/box, Pick a technique → #techniques) still clickable. Evidence: before `breathing-visualizer-mobile-orb-dead.png`, after `FIX25-478timer-mobile-orb-now-starts.png`.
