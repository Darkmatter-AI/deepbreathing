...
**Hypothesis:** Signed-in users who can see their practice history (total minutes, sessions, streak, a full-month practice calendar) will return more often and be harder to churn — the page makes their investment visible, reinforcing the identity "I am someone who breathes." Secondary: the signed-out value-prop view of the page nudges first-time visitors who are close to signing up.

**Change:** New page at `/stats` (linked from footer). Server-rendered for signed-in users: clickable profile avatar (links home), total minutes, sessions completed, live streak (0 if last_session_date older than yesterday — stale-streak fix), and a **full calendar-month grid** of real practice days. Per-day history is stored in a new `user_active_days` table (migration `003`), written on each stats sync and seeded on guest→user merge; the calendar falls back to the streak window if the table isn't migrated/backfilled yet. Also shows the user's saved breathing pattern (from `user_settings.mode`, labelled "Favorite pattern" per request — note this reflects the saved setting, not computed usage frequency, since no per-pattern counts are tracked). Signed-out state: value-prop copy + sign-in sheet CTA. A full per-session mode breakdown is still deferred — `session_events` (PHASE-0-SPEC Task 0.3) is intentionally not built here; `user_active_days` is a lean, forward-compatible store the calendar can later derive from it.
