...
11. **Post-deploy "skew window" = the real cause of "language resets / Application error on translated pages".** The MassTranslate proxy caches translated HTML, which embeds versioned `/_next/static/.../page-<hash>.js` chunk URLs. When the app redeploys, those hashes change but the proxy keeps serving the previous build's HTML for `/ja/...` etc. until its cache catches up. During that window the locale page either crashes (ChunkLoadError → React #423 → "Application error") or reverts to English on soft navigation. **It is transient and self-heals**, which is why it looked fixed and kept "regressing" — each deploy reopens it. Verified 2026-06-01: `/ja/breathe/coherent` served chunk `cb496ce7…`(404) while EN served `617e571a…`(200); both matched again ~minutes later. Don't patch this in app code (a client-side nav guard makes it *worse* — it forces a full reload of the stale-chunk HTML, turning a survivable revert into a guaranteed crash).

**Confirmed mechanism (traced into the edge-proxy, `mass-translate-backend/edge-proxy`).** The Cloudflare Worker caches translated HTML in `caches.default` for `cache_ttl` (default **86400s / 24h**), keyed by `…/${tenantId}/${lang}/${cacheVersion}/${pathHash}` (`src/utils.ts` `buildCacheKey`, `src/cache.ts`). `cache_version` is bumped by `TenantService.invalidate_cache` — but only on **translation** deploys (`deploy_translations`). An **app** deploy on Vercel changes the `/_next` chunk hashes without bumping `cache_version`, so the 24h-cached HTML keeps pointing at chunks the new build deleted → 404 → crash. A cache MISS is cheap (re-applies stored per-segment translations via `/api/proxy/translations`, no LLM), so invalidating freely is safe. Tenant id: `deepbreathingexercises_com_ac8ae5`. Vercel **Skew Protection: ENABLED 2026-06-02, Maximum Age 7 days** (`skewProtectionMaxAge: 604800`) — comfortably ≥ the 24h proxy `cache_ttl`, so old chunks outlive the stale cached HTML. Note: it only protects deployments created *after* it was turned on; the next deploy onward is covered.

**Durable fix (pick; they compose):**
1. **Bump `cache_version` on every app deploy** — `POST {API}/api/proxy/tenants/deepbreathingexercises_com_ac8ae5/invalidate-cache` (auth: owner session / API key). Closes the window in seconds. Wire it from a post-deploy step (GitHub Action on production `deployment_status`).
2. **Enable Vercel Skew Protection** (Project Settings → Advanced) with retention ≥ `cache_ttl`. Makes the crash structurally impossible (old chunks outlive the stale HTML). If the plan caps retention below 24h, also lower the tenant `cache_ttl` to match. Lowest-effort standalone fix.
3. (Platform, propose — don't ship unilaterally) make the proxy cache key build-aware so it self-heals for all tenants without a deploy signal.

**Detect it:** `pnpm check:locale-build` (`scripts/check-locale-build-consistency.mjs`) fails when a chunk referenced by a locale page returns non-200 *and still 404s after a settle re-check* (so it doesn't cry wolf on the benign version drift seen mid-rollout — the origin serves mixed builds for a minute or two after deploy). EN-vs-locale drift is logged as INFO only. Runs every 15 min + post-deploy via `.github/workflows/locale-build-consistency.yml`. Guards the crash/skew window, not the (RSC-layer) silent English revert.
