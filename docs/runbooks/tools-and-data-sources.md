...
- `mcp__mass-translate-backend__submit_urls_bing` is redundant: `postbuild` submits every sitemap URL to **IndexNow** on each Vercel production deploy (`scripts/ping-sitemap-lib.mjs`). Preview and development builds fail closed unless `VERCEL=1` and `VERCEL_ENV=production`, so they never notify search engines. Google does not participate in IndexNow. (The original submission path became active on 2026-07-10 after fixing the old `CI === "true"` mismatch; the production-only guard was added on 2026-07-15 after a preview exposed the broader CI gate.)
...
```
`GET /emails` lists **account-wide** and does NOT honour `domain`/`recipient` filters — you can't easily pull just our domain's sends via API. For per-domain delivery receipts use the dashboard Logs filtered by domain (`deepbreathingexercises.com` domain id `08d3cefb-de71-43f5-b40d-0df562d1b27b`). `?limit=100` works and returns `last_event` per message (`delivered` / `bounced` / `opened`), which is enough to confirm "is this email firing at all" — filter client-side on `from` containing the domain. The `?after=<id>` cursor **403s**, so you get one page; for a longer window use the dashboard.

**⚠️ `deepbreathingexercises.com` can SEND but cannot RECEIVE.** The apex has **no MX record** (Vercel DNS, `ns1/ns2.vercel-dns.com`), so `abi@` and `noreply@` there are unroutable — senders fall back to the A record per RFC 5321 and port 25 is closed on both Vercel IPs. Mail to those addresses bounces back to the sender. This silently ate every user reply to the welcome email from launch until 2026-07-25; `replyTo` now points at `hi@abiassi.com` (iCloud MX, verified receiving). **Never put a `@deepbreathingexercises.com` address in a `replyTo` or anywhere you expect a human response** unless MX records get added. Verify any reply address before shipping: `dig +short MX <domain>` must return records.

**Apple private-relay recipients bounce.** All `@privaterelay.appleid.com` users are in `email_suppressions` as bounces. Apple rejects mail whose sender address isn't registered under *Apple Developer → Certificates, IDs & Profiles → Sign in with Apple for Email Communication*. Both `abi@` and `noreply@deepbreathingexercises.com` need registering there to reach Apple-relay signups.
...
13. **The shared audio engine (`@resonance/audio`) runs on react-native-audio-api on iOS** (since 2026-07-22): same WebAudio code as web, driven natively by `apps/mobile/src/breathing/native-soundscape.ts` off webview events. Gotchas: (a) RNAA doesn't pull side-branch AnalyserNodes, so `getMeterValues()` reads -120dB on native even while audio plays — verify liveness via the `[soundscape]` dev log's `t=`/gain probes instead (ctx.currentTime must advance, layer gains must ramp to targets); (b) DynamicsCompressor and HRTF PannerNode are unimplemented in RNAA 0.13 — the engine auto-falls back to a WaveShaper soft-clip and StereoPanner orbit; upgrade when RNAA ships the real nodes; (c) RNAA renders repeated `setTargetAtTime` re-scheduling as FM warble (the 2026-07-22 'nightmare audio' root cause) — continuously-retargeted params must go through the engine's `glideParam()` (linear ramps on native via the `preferLinearRamps` platform flag); (d) RNAA contexts run at 48kHz — spectral analysis of captured PCM must use 48k or every pitch reads ~120 cents flat.
