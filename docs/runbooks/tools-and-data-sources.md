...
21. **`CRON_SECRET` is a *Sensitive* Vercel env var — `vercel env pull` returns it blank, so you cannot manually trigger `/api/warm-cache` from the CLI.** The route requires `Authorization: Bearer $CRON_SECRET` (or `?token=`) and returns 401 without it. `vercel env pull` writes `CRON_SECRET=` with an **empty** value because Vercel doesn't decrypt Sensitive vars back to the CLI (the line exists — `grep -c '^CRON_SECRET='` returns 1 — but the value is empty; that's the trap that wastes time). The 2 h Vercel Cron still warms fine (Vercel injects the secret server-side). To warm manually: trigger the cron run from the Vercel dashboard, or use the browser-fetch path in #22. (2026-06-15)

22. **Bulk-warming locale pages with a spoofed Googlebot UA from a local IP trips the anti-spoof 503 (refines #16d) — warm from a real browser instead.** A local `curl` loop spoofing Googlebot at concurrency 6 still 503'd ~62/265 locale URLs (anti-spoofing is per-source-IP and harsher on non-Vercel egress). Reliable manual-warm paths that avoid it: (a) the **2 h Vercel Cron** / dashboard cron-run — hits the real bot-path KV from Vercel's egress; (b) a **real browser** on `https://deepbreathingexercises.com` running a `fetch()` loop over the locale sitemap URLs (use `claude-in-chrome` + `javascript_tool`) — the genuine browser UA isn't spoofed so it doesn't 503, and it warms the expensive Vercel-origin SSR (the dominant cold cost in #17). Caveat: browser-UA warming does NOT warm the bot-path translation KV (only the Googlebot-UA cron path does), so it's a complement, not a replacement. (2026-06-15)
