...
**Resolution (settled):** hotfix PR #58 (commit `54eb173`) restored `[contenthash]` on async chunk filenames, and the follow-up commit removed the entire webpack block in favor of Next defaults. Both landed on `main` when PR #58 merged on 2026-07-22 (merge commit `9249ac2`); `next.config.js` on `main` carries no custom webpack block. Fresh HTML referenced fresh hashed URLs, so all poisoned browsers self-healed on next load — no cache purge was needed. First-load JS regressed part of the 27% win; correctness and debuggability won. The `hotfix/async-chunk-content-hash` branch was deleted on 2026-07-27 as fully superseded — do not resurrect it; its content is on `main` under different SHAs.
